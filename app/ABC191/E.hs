...
main = do
    [n, m] <- getIntList
    edges <- getTuple3s m
    let g = wgraph (n + 1) edges
        rg = wgraph (m + 1) $ VU.map (\(x, y, w) -> (y, x, w)) edges
    Monad.forM_ [1 .. n] $ \i -> do
        let ret = case map snd . filter (\j -> fst j == i) $ wgVertexes g i of
                xs | null xs -> -1
                xs           -> minimum xs
            fo = wbfs g i
            ba = wbfs rg i
            other = case VU.filter (\i -> i /= 0 && i /= -1) $ VU.zipWith (\i j -> if (i /= -1 && j /= -1) then i + j else -1) fo ba of
                xs | VU.null xs -> -1
                xs              -> VU.minimum xs
            result = case filter (\i -> i /= 0 && i /= -1) $ [ret, other] of
                xs | null xs -> -1
                xs           -> minimum xs
        print result
----------------
...
newtype WeightGraph a = WeightGraph (V.Vector [(Int, a)]) deriving (Show, Eq)

-- | O(|V| + |E|)? たぶん
wgraph :: VU.Unbox a => Int -> VU.Vector (Int, Int, a) -> WeightGraph a
wgraph n edges = WeightGraph $ V.modify (mutableWeightGraphEdgesApply edges) $ V.replicate n []

mutableWeightGraphEdgesApply :: (Prim.PrimMonad m, VU.Unbox a) => VU.Vector (Int, Int, a) -> VM.MVector (Prim.PrimState m) [(Int, a)] -> m ()
mutableWeightGraphEdgesApply edges mv = do
    VU.forM_ edges $ \edge -> do
        let (x, y, w) = edge
        VM.modify mv ((y, w):) x

-- | O(|E|) bfs
wbfs :: (VU.Unbox a, Num a, Eq a, Ord a) => WeightGraph a -> Int -> VU.Vector a
wbfs (WeightGraph v) i = ST.runST $ do
    visited <- VUM.replicate (V.length v) (-1)
    q <- queue (V.length v) 0
    enqueue q i
    VUM.write visited i 0
    while (qempty q) $ do
        x <- dequeue q
        dist <- VUM.read visited x
        Monad.forM_ (v V.! x) $ \(y, w) -> do
            connectedDist <- VUM.read visited y
            case connectedDist of
                x | x == -1 || x > dist + w -> do
                    enqueue q y
                    VUM.write visited y $ dist + w
                _ -> return ()
    VU.freeze visited

wgVertexes :: WeightGraph a -> Int -> [(Int, a)]
wgVertexes (WeightGraph v) i = v V.! i
