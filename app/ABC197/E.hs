...
main = do
    n <- getInt
    ps <- getTuple2s n
    let xs = VU.modify (VAM.sortBy (\(_, c0) (_, c1) -> compare c0 c1)) ps
    print $ solve xs 0

takesame xs = (VU.takeWhile (\(_, c) -> c == c0) xs, VU.dropWhile (\(_, c) -> c == c0) xs) where
    (x0, c0) = VU.head xs

solve xs pos
    | otherwise = VU.map (abs . (- pos) . fst) ys where
        (ys, zs) = takesame xs
