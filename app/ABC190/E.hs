...
main = do
    ...
    k <- getInt
    tar <- getVector
    let g = graph (n + 1) edges
    let dists = V.generate k $ bfs g . (VU.!) tar
    let m = mgenerate k k $ \i j -> (dists V.! i) VU.! (tar VU.! j)
    if VU.elem (-1) (mToVector m) then print (-1) else do
        result <- dp2 (2 ^ k) k (solve m)
        print $ 1 + minimum (map (mindex result (2 ^ k - 1)) [0 .. k - 1])
    return ()

bitToList :: Int -> [Int]
bitToList x = [n | n <- [0 .. floor . logBase 2 . fromIntegral $ x] , Bits.testBit x n]

solve :: Matrix Int -> MMatrix (Prim.PrimState IO) Int -> Int -> Int -> IO Int
solve _ mm s j | s == 0 || Bits.clearBit s j == 0 = return 0
solve m mm s j = do
    let prev = Bits.clearBit s j
    minimum <$> Monad.forM (bitToList prev) (\i -> (mindex m i j +) <$> mmread mm prev i)
