...
main = do
    [h, w] <- getIntList
    m <- BS8.concat <$> Monad.replicateM h BS8.getLine
    let result = VU.sum . VU.map f $ VU.fromList [(i, j) | i <- [0 .. (h - 2)], j <- [0 .. (w - 2)]] where
            f (i, j) = solve [BS8.index m (i * w + j), BS8.index m (i * w + j + 1), BS8.index m ((i + 1) * w + j), BS8.index m ((i + 1) * w + j + 1)]
    print result
    return ()

solve :: [Char] -> Int
solve str = if s == 3 || s == 1 then 1 else 0 where
    s = count (== '#') str
