...
main = do
    [n, k] <- getIntList
    print . (VU.! k) $ VU.iterateN (k + 1) f n

vint v | VU.null v = 0
vint v = VU.head v + 10 * vint (VU.tail v)

intv :: (VUM.Unbox a, Integral a) => a -> VU.Vector a
intv n = VU.generate ((floor . logBase 10 . fromIntegral $ n) + 1) (\i -> (n `mod` (10 ^ (i + 1))) `div` (10 ^ i))

f :: Int -> Int
f n = x where
    s = VU.modify VAM.sort $ intv n
    t = VU.reverse s
    x = vint s - vint t
