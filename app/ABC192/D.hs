...
main :: IO ()
main = do
    x <- V.reverse . V.map (toInteger . Char.digitToInt) . V.fromList <$> getLine
    m <- toInteger <$> getInt
    let len = V.length x
        d = V.maximum x
    if len >= 4 then do
        let r = Maybe.fromMaybe -1 $ List.find (\n -> vint n x > m) [d + 1 ..]
        print $ r - (d + 1)
    else if len == 1 then
        print $ if (m >= x V.! 0) then 1 else 0
    else if len == 2 then
        print $ case (m - x V.! 0) `div` (x V.! 1) of
            x | x < d + 1 -> 0
            x | otherwise -> x - d
    else do
        let up = floor . sqrt . fromIntegral $ (m `div` x V.! 2)
            r = Maybe.fromMaybe d  $ List.find (\n -> vint n x <= m) [up, up - 1 .. d + 1]
        print $ r - d

vint :: Integer -> V.Vector Integer -> Integer
vint _ v | V.null v = 0
vint n v = V.head v + n * vint n (V.tail v)
