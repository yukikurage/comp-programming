...
main = do
    n <- getInt
    xs' <- VU.fromList . map Char.digitToInt <$> getLine
    let xs = VU.generate n (\i -> ((xs' VU.! i) * 10 ^ ((n - i - 1) `mod` 6)) `mod` 7)
    ys <- VU.fromList <$> getLine
    putStrLn $ if solve n xs ys then "Takahashi" else "Aoki"
    return ()

solve :: Int -> VU.Vector Int -> VU.Vector Char -> Bool
solve n xs ys = dp A.! (0, 0) where
    dp = A.listArray ((0, 0), (n, 6)) [f (i, j) | i <-[0 .. n], j <- [0 .. 6]] :: A.Array (Int, Int) Bool
    f (i, 0) | i == n = True
    f (i, _) | i == n = False
    f (i, j) = (if ys ! i == 'T' then (||) else (&&)) (dp A.! (i + 1, j)) (dp A.! (i + 1, (j + (xs ! i)) `mod` 7))
