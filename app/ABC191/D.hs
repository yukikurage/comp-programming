...
main = do
    str <-BS8.getLine
    let [x, y, r] = map (fromIntegral . floor . (* 10000) . read . BS8.unpack) $ BS8.splitWith (== ' ') str :: [Double]
    print . VU.sum . VU.map (\t -> 1 + (floor (sqrt (r ^ 2 - (t - x) ^ 2) + y) :: Int) - ceiling (y - sqrt (r ^ 2 - (t - x) ^ 2))) . VU.map fromIntegral $ VU.fromList ([ceiling (x - r)  .. floor (x + r)] :: [Int])
