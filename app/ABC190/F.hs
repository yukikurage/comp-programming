...
main = do
    n <- getInt
    xs <- getVector
    resultRef <- IORef.newIORef 0
    t <- newmcbt (+) n 0
    VU.forM_ xs $ \x -> do
        y <- mcbtfold t x
        IORef.modifyIORef' resultRef (+ y)
        mcbtwrite t x 1
    result <- (((n * (n - 1)) `div` 2) -) <$> IORef.readIORef resultRef
    let ans = VU.map snd $ VU.iterateN n f (0, result) where
            f (i, x) = (i + 1, x + n - 1 - 2 * xs VU.! i)
    VU.forM_ ans print
