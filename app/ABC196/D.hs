...
main = do
    [h, w, a, b] <- getIntList
    print . solve h w a b . AU.listArray ((0, 0), (h - 1, w - 1)) $ [True, True ..]
    return ()

solve :: Int -> Int -> Int -> Int -> AU.UArray (Int, Int) Bool -> Int
solve h w 0 0 xs | isFilled xs = 1
solve h w a b xs | a < 0 || b < 0 = 0
solve h w a b xs =
    solve h w a (b - 1) (xs A.// [(i, False)]) +
    (if snd i + 1 <= w - 1 then solve h w (a - 1) b $ xs A.// [(i, False), (i *+* (0, 1), False)] else 0) +
    (if fst i + 1 <= h - 1 then solve h w (a - 1) b $ xs A.// [(i, False), (i *+* (1, 0), False)] else 0)
    where
        i = Maybe.fromJust $ List.find (xs A.!) $ A.range ((0, 0), (h - 1, w - 1))

isFilled :: AU.UArray (Int, Int) Bool -> Bool
isFilled xs = not . or $ A.elems xs
