...
main = do
    ...
    xs <- getVector
    let seps = sep xs
    print . minimum . map f $ seps
    return ()

f :: [VU.Vector Int] -> Int
f xs = foldr1 Bits.xor . map g $ xs

g :: VU.Vector Int -> Int
g xs = VU.foldr1' (Bits..|.) xs

sep :: VU.Vector Int -> [[VU.Vector Int]]
sep xs
    | VU.length xs == 1 = [[xs]]
    | otherwise = map (\(w : ws) -> VU.cons h w : ws) (sep ys) ++ map (VU.singleton h :) (sep ys) where
        h = VU.head xs
        ys = VU.tail xs
